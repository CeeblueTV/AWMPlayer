var AwmUtil = {
  ...
  array: {
    ...
    multiSort: function (array, sortby) {
      /*
       AwmUtil.array.multiSort([].concat(video.info.source),[
  ...
  http: {
    getpost: function (type, url, data, callback, errorCallback) {
      ...
      if (errorCallback) {
        xhr.timeout = 8e3;
      } //go to timeout function after 8 seconds
  ...
  scripts: {
    list: {},
    insert: function (src, onevent, AwmVideo) {
      var scripts = this;

      if (AwmVideo) {
        //register so we can remove it on unload
        AwmVideo.errorListeners.push({
          src: src,
          onevent: onevent
        });
      }
      ...
      for (var type in output) {
        ...
        for (var i in output[type]) {
          if (!equal) {
            ...
            equal = AwmUtil.object.extend({}, output[type][i].describe);
            continue;
          }
          if (AwmUtil.object.keys(output[type]).length > 1) {
